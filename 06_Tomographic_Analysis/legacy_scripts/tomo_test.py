...
#%%
# --- USER PARAMETERS FOR TROUBLESHOOTING ---
# Path to .mat files
fft_mat_path = '/scratch/2025_Feb/temp/FFT_RS_128.mat'
cellinfo_mat_path = '/scratch/2025_Feb/temp/cellinfo_256_BL_2.mat'



# Which Qv columns to use for x, y, z (0, 1, 2)
q_axes = [1, 2, 0]  # try [2,1,0] or [0,2,1] if axes are swapped
q_signs = [1, 1, 1] # try [-1,1,1] etc. to flip axes
flatten_order = 'C'  # 'C' (row-major) or 'F' (column-major)
...
# --- LOAD DATA ---
fft_data = sio.loadmat(fft_mat_path)
tomo_data_RS = fft_data['DATA_m']
tomo_data_RS_qs = fft_data['Qv_m']
tomo_data_RS[np.isnan(tomo_data_RS)] = 0
tomo_data_RS_qs[np.isnan(tomo_data_RS_qs)] = 0

cellinfo_data = sio.loadmat(cellinfo_mat_path)
if 'cellinfo' in cellinfo_data:
    cellinfo_data = cellinfo_data['cellinfo']

# --- PREPARE Q ARRAYS ---
# Qv is (N,3) or (nx*ny*nz,3) or similar
# If Qv is 2D, try to reshape to 3D if needed
Q = tomo_data_RS_qs
if Q.ndim == 2 and Q.shape[1] == 3:
    # Try to infer shape from tomo_data_RS
    npts = np.prod(tomo_data_RS.shape)
    if Q.shape[0] == npts:
        Qx = Q[:, q_axes[0]].reshape(tomo_data_RS.shape, order=flatten_order) * q_signs[0]
        Qy = Q[:, q_axes[1]].reshape(tomo_data_RS.shape, order=flatten_order) * q_signs[1]
        Qz = Q[:, q_axes[2]].reshape(tomo_data_RS.shape, order=flatten_order) * q_signs[2]
    else:
...
magnitude_flat = tomo_data_RS.flatten(order=flatten_order)

# --- THRESHOLD ---
thresh_val = threshold * np.max(magnitude_flat)
mask = magnitude_flat > thresh_val

print(f"Loaded Q arrays with shape: {Q.shape}")
print(f"First {print_n} Q-points (x, y, z):")
for i in range(print_n):
    print(f"  {i}: ({kx_flat[i]:.4f}, {ky_flat[i]:.4f}, {kz_flat[i]:.4f})")

# --- GENERATE UNIT CELL PEAKS ---
def generate_miller_indices(max_order=1):
    indices = []
...
miller_indices = generate_miller_indices(miller_order)
hs = miller_indices[:, 0]
ks = miller_indices[:, 1]
ls = miller_indices[:, 2]

# --- RECIPROCAL LATTICE VECTORS ---
recilatticevectors = cellinfo_data['recilatticevectors'][0,0] if isinstance(cellinfo_data, np.ndarray) else cellinfo_data['recilatticevectors']
vs = []
for i, h in enumerate(hs):
    v = (
        hs[i] * recilatticevectors[0] +
        ks[i] * recilatticevectors[1] +
        ls[i] * recilatticevectors[2]
    )
    vs.append(v)
vs = np.array(vs)

print(f"First {print_n} unit cell peaks:")
for i in range(print_n):
    print(f"  {i}: ({vs[i,0]:.4f}, {vs[i,1]:.4f}, {vs[i,2]:.4f})")
...
fig.add_trace(go.Scatter3d(
    ...
    z=kz_flat[mask],
    mode='markers',
    marker=dict(size=6, color=magnitude_flat[mask], colorscale='Viridis', opacity=0.4, colorbar=dict(title='Magnitude')),
    name='FFT Q-points'
))
fig.add_trace(go.Scatter3d(
    x=vs[:,0],
...
fig.update_layout(
    title="FFT Q-points and Unit Cell Peaks (TROUBLESHOOT MODE)",
    scene=dict(
        xaxis_title="Qx (Å⁻¹)",
        yaxis_title="Qy (Å⁻¹)",
        zaxis_title="Qz (Å⁻¹)",
...
print("- Try changing q_axes, q_signs, or flatten_order at the top of the script.")
print("- Compare the above Q-points and peaks to your MATLAB output.")
print("- If axes are swapped, try q_axes=[2,1,0] or similar.")
print("- If signs are flipped, try q_signs=[-1,1,1] or similar.")
print("- If points are scrambled, try flatten_order='F'.") 
...
if lattice_name=='ClathII':
    if loss_type=='pearson_loss':
        ...
        thresholds=[0.0001, 0.102] #pearson 250 epochs 60/67, 1 false positive
    elif loss_type=='L2':
...
# --- CREATE 3D TOMOGRAM WITH GAUSSIAN BLOBS AT INDEXED PEAKS ---
def add_gaussian_blob(volume, center, sigma, grid_x, grid_y, grid_z):
    """
    Adds a 3D Gaussian blob to the volume at the specified center.
    Args:
        volume: 3D numpy array to add the blob to
        center: (x, y, z) coordinates of the blob center (in Q-space)
        sigma: standard deviation of the Gaussian
        grid_x, grid_y, grid_z: 3D meshgrid arrays matching the volume shape
    """
    blob = np.exp(-((grid_x - center[0])**2 + (grid_y - center[1])**2 + (grid_z - center[2])**2) / (2 * sigma**2))
    volume += blob
    return volume

# Use the first dataset's Qv as the grid
Qx = filtered_datasets[0]['Qx']
Qy = filtered_datasets[0]['Qy']
Qz = filtered_datasets[0]['Qz']
data_shape = filtered_datasets[0]['DATA'].shape
raw_data = filtered_datasets[0]['DATA']  # Get the raw data

# if Q.ndim == 2 and Q.shape[1] == 3 and Q.shape[0] == np.prod(data_shape):
#     Qx = Q[:, q_axes[0]].reshape(data_shape, order=flatten_order) * q_signs[0]
#     Qy = Q[:, q_axes[1]].reshape(data_shape, order=flatten_order) * q_signs[1]
#     Qz = Q[:, q_axes[2]].reshape(data_shape, order=flatten_order) * q_signs[2]
# else:
#     Qx = Q[..., q_axes[0]] * q_signs[0]
#     Qy = Q[..., q_axes[1]] * q_signs[1]
#     Qz = Q[..., q_axes[2]] * q_signs[2]

# Initialize empty tomogram
tomogram = np.zeros(data_shape, dtype=np.float32)

# Set the Gaussian width (in Q units)
sigma = 0.005/5  # adjust as needed

# Add a blob for each close peak
for peak in close_peaks:
    tomogram = add_gaussian_blob(tomogram, peak, sigma, Qx, Qy, Qz)


factor = 2  # or 2, depending on your data size
Qx_ds = Qx[::factor, ::factor, ::factor]
Qy_ds = Qy[::factor, ::factor, ::factor]
Qz_ds = Qz[::factor, ::factor, ::factor]
tomogram_ds = tomogram[::factor, ::factor, ::factor]

fig_blob = go.Figure(data=go.Isosurface(
    x=Qx_ds.flatten(),
    y=Qy_ds.flatten(),
    z=Qz_ds.flatten(),
    value=tomogram_ds.flatten(),
    isomin=0.001 * np.max(tomogram_ds),
    isomax=np.max(tomogram_ds),
    opacity=0.6,
    surface_count=3,
    colorscale='Hot'
))
...
#     isomin=0.1 * np.max(tomogram),
#     isomax=np.max(tomogram),
#     opacity=0.6,
#     surface_count=3,
#     colorscale='Hot'
# ))
fig_blob.add_trace(go.Scatter3d(
    x=close_peaks[:,0],
    y=close_peaks[:,1],
    z=close_peaks[:,2],
    mode='text',
...
# Example: list of hkl labels to highlight
highlight_hkls = ['(2,0,0)', '(6,0,0)', '(0,0,-2)', '(0,0,-6)',
                  '(0,0,2)', '(0,0,6)', '(-2,0,0)', '(-6,0,0)']  # Replace with your desired labels
...
# --- FUNCTION TO OVERLAY Qz SLICE OF GAUSSIAN BLOBS AND RAW DATA ---
def plot_overlay_slice(Qx, Qy, Qz, raw_data, tomogram, qz_value=None, qz_index=None, cmap_raw='gray', cmap_blob='hot', alpha_blob=0.5):
    """
    Overlay a slice along Qz from both the tomogram (Gaussian blobs) and the raw data.
    Args:
        Qx, Qy, Qz: 3D numpy arrays of Q coordinates (same shape as data)
        raw_data: 3D numpy array (filtered raw data)
        tomogram: 3D numpy array (Gaussian blobs)
        qz_value: Qz value to slice at (finds closest index)
        qz_index: Direct index to slice at (overrides qz_value)
        cmap_raw: Colormap for raw data
        cmap_blob: Colormap for blob overlay
        alpha_blob: Transparency for blob overlay
    """
    import matplotlib.pyplot as plt
    import numpy as np

    # Determine slice index
    if qz_index is not None:
        idx = qz_index
        qz_actual = Qz[0,0,idx]
    elif qz_value is not None:
        # Find the index closest to qz_value
        qz_axis = Qz[0,0,:]
        idx = np.argmin(np.abs(qz_axis - qz_value))
        qz_actual = qz_axis[idx]
    else:
        raise ValueError("Must specify either qz_value or qz_index.")

    # Extract 2D slices
    raw_slice = raw_data[:,:,idx]
    blob_slice = tomogram[:,:,idx]
    qx_slice = Qx[:,:,idx]
    qy_slice = Qy[:,:,idx]

    plt.figure(figsize=(8,6))
    plt.imshow(raw_slice.T, origin='lower', cmap=cmap_raw,
               extent=[qx_slice.min(), qx_slice.max(), qy_slice.min(), qy_slice.max()])
    plt.colorbar(label='Raw Data Intensity')
    plt.imshow(blob_slice.T, origin='lower', cmap=cmap_blob, alpha=alpha_blob,
               extent=[qx_slice.min(), qx_slice.max(), qy_slice.min(), qy_slice.max()])
    plt.title(f'Overlay Slice at Qz = {qz_actual:.4f} (index {idx})')
    plt.xlabel('Qx (Å$^{-1}$)')
    plt.ylabel('Qy (Å$^{-1}$)')
    plt.tight_layout()
    plt.show()

# Example usage: overlay at the central Qz slice
nz = Qz.shape[2]
center_idx = nz // 2
plot_overlay_slice(Qx, Qy, Qz, raw_data, tomogram, qz_index=center_idx)
# Or, to specify a Qz value:
# plot_overlay_slice(Qx, Qy, Qz, raw_data, tomogram, qz_value=0.0)
...
#%%
def plot_spherical_average(data, Qx, Qy, Qz, nbins=200, label=None, color=None, eps=1e-12, background_subtract=True, q_min_cut=None, q_max_cut=None, scale_by_q4=False):
    """
    Compute and plot the normalized spherical average (radial profile, log scale) of a 3D tomogram in reciprocal space.
    Args:
        data: 3D numpy array (intensity)
        Qx, Qy, Qz: 3D numpy arrays (same shape as data)
        nbins: number of bins for |Q|
        label: label for the plot
        color: color for the plot
        eps: small value to avoid log(0)
        background_subtract: if True, subtract mean of highest 10% q bins as flat background
        q_min_cut: minimum |Q| to plot (float or None)
        q_max_cut: maximum |Q| to plot (float or None)
        scale_by_q4: if True, scale the radial average by 1/q^4 before normalization
    Returns:
        bin_centers, radial_avg
    """
    q_mag = np.sqrt(Qx**2 + Qy**2 + Qz**2)
    q_flat = q_mag.flatten()
    data_flat = data.flatten()
    # Remove NaNs
    mask = ~np.isnan(q_flat) & ~np.isnan(data_flat)
    q_flat = q_flat[mask]
    data_flat = data_flat[mask]
    # Bin by |Q|
    q_min, q_max = np.min(q_flat), np.max(q_flat)
    bins = np.linspace(q_min, q_max, nbins+1)
    bin_centers = 0.5 * (bins[:-1] + bins[1:])
    radial_avg = np.zeros(nbins)
    for i in range(nbins):
        in_bin = (q_flat >= bins[i]) & (q_flat < bins[i+1])
        if np.any(in_bin):
            radial_avg[i] = np.mean(data_flat[in_bin])
        else:
            radial_avg[i] = np.nan
    # Background subtraction
    if background_subtract:
        n_bg = max(1, nbins // 10)
        bg = np.nanmean(radial_avg[-n_bg:])
        radial_avg = radial_avg - bg
        radial_avg[radial_avg < 0] = eps
    # Scale by 1/q^4 if requested
    if scale_by_q4:
        radial_avg = radial_avg * (bin_centers**4 + eps)
    # Normalize
    radial_avg_norm = radial_avg / (np.nanmax(radial_avg) + eps)
    # Q range cutoff
    plot_mask = np.ones_like(bin_centers, dtype=bool)
    if q_min_cut is not None:
        plot_mask &= (bin_centers >= q_min_cut)
    if q_max_cut is not None:
        plot_mask &= (bin_centers <= q_max_cut)
    plt.plot(bin_centers[plot_mask], np.log10(radial_avg_norm[plot_mask] + eps), label=label, color=color)
    plt.xlabel(r'$|Q|$ (Å$^{-1}$)')
    ylabel = 'log$_{10}$(Normalized Spherical Average Intensity)'
    if scale_by_q4:
        ylabel += r' * $Q^4$'
    plt.ylabel(ylabel)
    plt.title('Spherical Average (Radial Profile, log scale)')
    plt.grid(True)
    if label is not None:
        plt.legend()
    return bin_centers[plot_mask], radial_avg_norm[plot_mask]

# Example usage after plot_multi_reciprocal_space:
...
for idx, dataset in enumerate(rs_datasets):
    ...
    if Q.ndim == 2 and Q.shape[1] == 3:
        ...
        Qy = Q[..., q_axes[1]] * q_signs[1]
        Qz = Q[..., q_axes[2]] * q_signs[2]
    plot_spherical_average(
        data, Qx, Qy, Qz, nbins=200,
        label=dataset.get('label', f'Dataset {idx+1}'),
        color=colors[idx%len(colors)],
        background_subtract=True,
        q_min_cut=0.02,
        q_max_cut=0.12,
        scale_by_q4=True  # enable 1/q^4 scaling
    )
plt.tight_layout()
plt.show()
