...
1. Ptychography entries: Lines with "#I detector_filename SZC2_3D_XXX_XXXXX_XXXXX.tif"
   followed by "#I phi = YYY.YYYY" lines
2. SAXS entries: Lines with "#I detector_filename SZC2SAXSXXX_XXXXX_XXXXX.tif"
...
class LogParser:
    ...
    def parse_log(self) -> None:
        ...
        while i < len(lines):
            ...
            # Look for ptychography detector filename entries
            if line.startswith('#I detector_filename') and 'SZC2_3D_' in line:
                ptycho_entry = self._parse_ptycho_entry(lines, i)
                if ptycho_entry:
                    self.ptycho_entries.append(ptycho_entry)
            
            # Look for SAXS detector filename entries
            elif line.startswith('#I detector_filename') and 'SZC2SAXS' in line:
    ...
    def _parse_ptycho_entry(self, lines: List[str], start_idx: int) -> Optional[PtychoEntry]:
        ...
        # Extract filename components: SZC2_3D_323_00031_00071.tif
        filename_match = re.search(r'SZC2_3D_(\d+)_(\d+)_(\d+)\.tif', detector_line)
    ...
    def _parse_saxs_entry(self, lines: List[str], start_idx: int) -> Optional[SAXSEntry]:
        ...
        # Extract filename components: SZC2SAXS344_00001_00721.tif
        filename_match = re.search(r'SZC2SAXS(\d+)_(\d+)_(\d+)\.tif', detector_line)
        if not filename_match:
            return None
...
def load_ptycho_raw_diffraction_sum(scan_number: int, crop_size: int = 256, 
    ...
    pattern = f"/net/micdata/data2/12IDC/2025_Jul/ptycho/{scan_number}/SZC2_3D_{scan_number}_*.h5"
...
def load_saxs_data(scan_number: int, phi_index: int) -> Optional[np.ndarray]:
    """Load SAXS data from HDF5 file at specific phi index."""
    saxs_path = f"/net/micdata/data2/12IDC/2025_Jul/misc/combined_SZC2SAXS{scan_number}_00001.h5"
...
def run_correlation():
    ...
    ptycho_scan = 318#355#318                    # Ptychography scan number
    saxs_scan = 344#346#344                      # SAXS scan number
    ...
    # Filter by separate scan numbers
    ptycho_entries, saxs_entries = log_parser.filter_by_separate_scans(ptycho_scan, saxs_scan)
    ...
    if enable_plotting and matches:
        ...
        if ptycho_data is not None:
            ...
            if matches:
                ...
                saxs_data = load_saxs_data(saxs_scan, saxs_measurement.phi_index)
