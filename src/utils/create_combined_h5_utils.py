...
def process_scan(scan_number, base_path, model, device, center, dpsize, mask, indices_file=None, exp_data=True, sim_dps=None):
    """Process a single scan
    
    Parameters:
    -----------
    scan_number : int
        Scan number (for experimental data) or pattern index (for simulated data)
    base_path : str
        Base path for experimental data files (ignored if exp_data=False)
    model : torch.nn.Module
        Trained model for deconvolution
    device : torch.device
        Device to run model on
    center : tuple, optional
        (row, col) center position for cropping. Can be None for simulated data (patterns already correct size)
    dpsize : int
        Size of diffraction pattern to crop
    mask : numpy.ndarray
        Mask for preprocessing
    indices_file : str, optional
        Path to H5 file with selected indices (for experimental data only)
    exp_data : bool, default=True
        If True, load experimental data from base_path. If False, use sim_dps
    sim_dps : numpy.ndarray, optional
        Simulated diffraction patterns array (shape: [n_patterns, height, width])
        Required if exp_data=False
    """
    try:
        ...
        if exp_data:
            ...
            # Simulated data: use sim_dps directly
            if sim_dps is None:
                raise ValueError("sim_dps must be provided when exp_data=False")
            # For simulated data, scan_number can be:
            # - An int: single pattern index (legacy support)
            # - A list/array: multiple pattern indices to process together (for summing)
            if isinstance(scan_number, (list, np.ndarray)):
                # Multiple patterns (list/array of indices) - will be summed
                pattern_indices = np.array(scan_number)
                dps = sim_dps[pattern_indices]
                print(f"Processing {len(pattern_indices)} patterns (indices {pattern_indices[0]}-{pattern_indices[-1]}) - will be summed")
            else:
                # Single pattern (int index) - legacy support
                dps = np.array([sim_dps[scan_number]])
                print(f"Processing simulated pattern {scan_number}")
        ...
        for i in range(n_batches):
            ...
            for j, dp in enumerate(batch_dps):
                # For simulated data, patterns are already the correct size (no cropping needed)
                # For experimental data, crop around center
                if center is not None and (dp.shape[0] > dpsize or dp.shape[1] > dpsize):
                    cropped_dp = dp[center[0]-dpsize//2:center[0]+dpsize//2,
                                  center[1]-dpsize//2:center[1]+dpsize//2]
                else:
                    # Pattern is already the correct size or center not provided (simulated data)
                    cropped_dp = dp
                raw_list[j] = cropped_dp
                resultT, sfT, bkgT = preprocess_ZCB_9(cropped_dp, mask)
...
def save_verification_plot(results, output_dir='plots'):
    ...
    try:
        ...
        plt.savefig(os.path.join(output_dir, f'scan_{results["scan_number"]}_summary.png'))
        plt.close()
    except Exception as e:
